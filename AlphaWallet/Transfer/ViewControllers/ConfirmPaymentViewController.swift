...
class ConfirmPaymentViewController: UIViewController {
    ...
    private let includeTxCost: Bool
    private let maxFund: BigInt?
    private var amountWithFee: BigInt?
    ...
    init(
        ...
        self.maxFund = session.balance?.value
        //self.isSendMax = isSendMax
        self.includeTxCost = includeTxCost
    ...
    private func configure(for detailsViewModel: ConfirmPaymentDetailsViewModel) {
        ...
        if let maxFund = maxFund {
            confirmEnabled = detailsViewModel.canPay(includeTxCost: includeTxCost, maxFund: maxFund)
        } else {
            confirmEnabled = true
        }
            
        if includeTxCost {
            amountWithFee = detailsViewModel.amountMinusFee
            header.configure(amount: detailsViewModel.amountAttributedStringWithFee)
        } else {
        ...
        button.isEnabled = confirmEnabled
    ...
    @objc func send() {
        displayLoading()
        let transaction = configurator.formUnsignedTransaction(valueDeductedWithTxFee: includeTxCost ? amountWithFee : nil)
